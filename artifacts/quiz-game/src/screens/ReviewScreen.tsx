...
import { AnswerRecord, getDifficultyBg } from "@/hooks/use-quiz";
...
export function ReviewScreen({ answers, onBack }: ReviewScreenProps) {
  ...
  return (
    <motion.div
      ...
      className="min-h-screen pt-20 pb-12 px-4 max-w-4xl mx-auto w-full"
    >
      {/* Header */}
            ...
            >
              ...
              <div className="pl-5 pr-5 pt-4 pb-5">
                ...
                <div className="flex items-center flex-wrap gap-2 mb-3">
                  ...
                  <span className={cn("text-xs font-bold px-2 py-0.5 rounded text-white", getDifficultyBg(question.difficulty))}>
                    {question.difficulty}
                  </span>
                  ...
                  <div className="ml-auto flex items-center gap-2 flex-wrap">
                    ...
                    {streakAtAnswer >= 3 && (
                      <span className="text-xs font-black text-orange-400">🔥 {streakAtAnswer}×</span>
                    )}
                    <span className="text-xs text-muted-foreground">{timeUsed.toFixed(0)}s</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  {!isCorrect && (
                    <div className={cn("p-3 rounded-xl border", isTimeout ? "bg-muted/50 border-border" : "bg-red-500/10 border-red-500/20")}>
                      <div className={cn("text-xs font-black mb-1 uppercase tracking-wider", isTimeout ? "text-muted-foreground" : "text-red-400")}>
                        {isTimeout ? "⏰ Not Answered" : "✗ Your Answer"}
                      </div>
                      <div className={cn("font-bold text-sm", isTimeout ? "text-muted-foreground italic" : "text-foreground")}>
